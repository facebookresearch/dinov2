...
class THEOPOULA_REPLICA(Optimizer):
    def __init__(self, params, lr=1e-2, eta=0, betas=None, r=3, eps=1e-8, weight_decay=0, swap_probability=0.5):
        """
        Args:
        ...
        # New parameter for Asynchronous Replica Exchange
    ...
    def step(self, closure=None):
        ...
        for i in range(len(self.theopoulas)-1):
            if torch.rand(1, device=device).item() < self.swap_probability:  # Probabilistic swap attempt
                self._attempt_swap(
                self.theopoulas[i], 
                self.theopoulas[i+1],
                losses[i],
        ...
        self._adjust_temperature_ladder()  # Adjust the temperature ladder after swaps
        if self.total_attempts % 10 == 0:
            self._log_detailed_stats()
        return losses[0]  # Return loss from highest inverse temperature
    
    def _attempt_swap(self, opt_a, opt_b, loss_a, loss_b, beta_a, beta_b, pair_idx, device):
        delta_beta = (beta_b - beta_a)
        delta_E = loss_b - loss_a
    ...
    def _adjust_temperature_ladder(self):
        """Adjust the temperature ladder based on acceptance rates."""
        i = 0
        while i < len(self.betas) - 1:
            acceptance_rate = self.get_acceptance_rate()[i]
            if acceptance_rate > 0.8:  # High acceptance rate, merge replicas
                if i < len(self.betas) - 2:  # Ensure there's a next replica to merge with
                    # Calculate the geometric mean for logarithmic spacing
                    merged_beta = (self.betas[i] * self.betas[i + 1]) ** 0.5  # Geometric mean
                    self.betas[i] = merged_beta  # Update the current beta
                    self.betas.pop(i + 1)  # Remove the next beta
                    self.theopoulas.pop(i + 1)  # Remove the corresponding optimizer
                else:
                    i += 1  # Move to the next beta if no merge is possible
            elif acceptance_rate < 0.2:  # Low acceptance rate, split replicas
                new_beta = (self.betas[i] * self.betas[i + 1]) ** 0.5  # New beta between current and next
                self.betas.insert(i + 1, new_beta)  # Insert new beta
                self.theopoulas.insert(i + 1, THEOPOULA(self.param_groups[0]['params'], lr=self.lr, eta=self.eta, beta=new_beta, r=self.r, eps=self.eps, weight_decay=self.weight_decay))  # New optimizer
                i += 1  # Move to the next beta after insertion
            else:
                i += 1  # Move to the next beta if no action is taken

        # Ensure the betas list remains sorted (though it should already be sorted)
        self.betas.sort(reverse=True)
