...
class DinoVisionTransformer(nn.Module):
    ...
    def get_last_self_attention(self, x, masks=None):
        ...
        if self.chunked_blocks:
            ...
            for i, block_chunk in enumerate(self.blocks):
                if i < len(self.blocks) - 1:
                    x = block_chunk(x)
                else:
                    for j, blk in enumerate(block_chunk):
                        if j < len(block_chunk) - 1:
                            x = blk(x)
                        else:
...
def vit_small(patch_size=16, num_register_tokens=0, **kwargs):
    model = DinoVisionTransformer(
        ...
        embed_dim=384,
        depth=12,
        num_heads=6,  # embed_dim per head is 64
        mlp_ratio=4,
...
def vit_base(patch_size=16, num_register_tokens=0, **kwargs):
    model = DinoVisionTransformer(
        ...
        embed_dim=768,
        depth=12,
        num_heads=12,  # embed_dim per head is 64
        mlp_ratio=4,
        block_fn=partial(Block, attn_class=MemEffAttention),
        num_register_tokens=num_register_tokens,
        **kwargs,
...
def vit_large(patch_size=16, num_register_tokens=0, **kwargs):
    model = DinoVisionTransformer(
        ...
        embed_dim=1024,
        depth=24,
        num_heads=16,  # embed_dim per head is 64
        mlp_ratio=4,
        block_fn=partial(Block, attn_class=MemEffAttention),
        num_register_tokens=num_register_tokens,
        **kwargs,
...
def vit_huge(patch_size=16, num_register_tokens=0, **kwargs):
    model = DinoVisionTransformer(
        ...
        embed_dim=1280,
        depth=32,
        num_heads=16,  # embed_dim per head is 80
        mlp_ratio=4,
        block_fn=partial(Block, attn_class=MemEffAttention),
        num_register_tokens=num_register_tokens,
        **kwargs,
